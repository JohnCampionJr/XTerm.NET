...
public class WideCellInvariantTests
{
    ...
    private static void AssertNoOrphans(Terminal t, int row = 0)
    {
        ...
        for (var x = 0; x < t.Cols; x++)
        {
            ...
            (x + 1 < t.Cols).Should().BeTrue($"wide cell at {x} has no room for its spacer");
            ((string.IsNullOrEmpty(line[x + 1].Content) || line[x + 1].Content == " ")).Should().BeTrue($"wide cell at {x} is followed by '{line[x + 1].Content}' instead of its spacer");
        }
    }
    ...
    public void A_wide_character_at_the_last_column_wraps_instead_of_being_crammed_in()
    {
        ...
        (terminal.Buffer.Lines[1]![0].Content).Should().Be("\u6F22");
    }
    ...
    public void A_cluster_never_measures_more_than_two_columns()
    {
        ...
        ((line[0].Width <= 2)).Should().BeTrue($"cluster measured {line[0].Width} columns");
    }
    ...
    public void A_zwj_only_keeps_the_cluster_for_a_pictograph()
    {
        ...
        var line = terminal.Buffer.Lines[0]!;
        line[0].Content.Should().NotContain("e");
        line[2].Content.Should().Be("e");
    }
    ...
    public void A_zwj_emoji_sequence_still_joins()
    {
        ...
        runes.Should().Equal([0x1F468, 0x200D, 0x1F469]);
    }
    ...
    public void An_ascii_run_does_not_leave_a_zwj_continuation_for_a_later_emoji_to_pick_up()
    {
        ...
        var line = terminal.Buffer.Lines[0]!;
        line[2].Content.Should().Be("\U0001F469");
        line[2].Width.Should().Be(2);
        line[0].Content.Should().Be("\U0001F468‍");
    }
    ...
    public void A_wide_character_at_the_right_margin_keeps_its_spacer_inside_the_pane()
    {
        ...
        for (var y = 0; y < 4; y++)
        {
            ...
            ((string.IsNullOrEmpty(row[12].Content) || row[12].Content == " ")).Should().BeTrue($"row {y} column 12 holds '{row[12].Content}', outside the right margin");
            AssertNoOrphans(terminal, y);
        }

        // It wrapped into the pane instead: left margin of the next row.
        (terminal.Buffer.Lines[1]![4].Content).Should().Be("界");
    }
    ...
    public void A_wide_character_that_cannot_fit_is_dropped_when_wrapping_is_off()
    {
        ...
        AssertNoOrphans(terminal);
        (terminal.Buffer.Lines[0]![9].Width).Should().NotBe(2);
    }
    ...
    public void A_wide_character_copied_into_a_line_is_still_repaired_afterwards()
    {
        ...
        line.HasWideCells.Should().BeTrue("a line holding a copied wide cell must report it, or its repairs are skipped");
    ...
    public void A_cloned_line_carries_the_wide_cell_latch()
    {
        ...
        (source.Clone().HasWideCells).Should().BeTrue();
    }
